...
listOfData=$(cat Scripts/annot_table_all.csv | tail -n +2 | cut -f 2 | sort | uniq)

for type in $listOfData;
do
  mkdir $type
  alignmentList=$(tail -n +2 /data/schwartzlab/eren/Chapter2/CONTIGS/Scripts/"$type"_table.csv | cut -f 1)
  for file in $alignmentList;
  do
    ln -s /data/schwartzlab/eren/Chapter2/SISRS_Run/contigs_outputs/${file} /data/schwartzlab/eren/Chapter2/CONTIGS/${type}/${file}
  done
done
